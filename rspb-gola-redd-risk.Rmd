...
terraOptions(tempdir = "~/repos/temp")
...
Level-1 classes in the GLanCE and TimeSpec4LULC datasets were recoded below to match class labels cited in the "Lookups" sheet of "ER_Workbook_Gola_Liberia.xlsx", and the report titled "Liberia's Forest Reference Emission Level Submission to the UNFCCC [@woodcockGlobalLandCover; @governmentofliberiaLiberiasForestReference2019]. Appending to the GLanCE datapoints, the Winrock remote sensing team derived additional training samples. This sampling aimed to improve class-to-country proportionality and considered homogenous rule and plot-to-pixel dimensions throughout sample selection.
...
samples_winrock$label <- case_when(
  str_detect(samples_winrock$Name, "^Bareland")  ~ "Bareground",
  str_detect(samples_winrock$Name, "^Regrowth")  ~ "Regrowth",
  str_detect(samples_winrock$Name, "^Farmbush")  ~ "Farmbush",
  str_detect(samples_winrock$Name, "^Water")     ~ "Water",
  str_detect(samples_winrock$Name, "^Swamp")     ~ "Swamp",
  TRUE ~ "MISSING"
  ) |> as.factor()
...
samples_winrock$code <- case_when(
  ...
  samples_winrock$label == "Water"      ~ 3,
  samples_winrock$label == "Swamp"      ~ 4,
...
forest_loss_2019_2024_estimate <- sum(forest_loss_2014_2019[],na.rm=T)*resolution^2/10000
forest_loss_2014_2024_estimate <- sum(forest_loss_2014_2019[],na.rm=T)*resolution^2/10000
...
cat("Total forest Loss across Liberia (2014-2019):", forest_loss_2014_2019_gola_estimate, "hectares\n")
cat("Total forest Loss across Liberia (2019-2024):", forest_loss_2014_2019_tong_estimate, "hectares\n")
cat("Total forest Loss across Liberia (2014-2024):", forest_loss_2014_2019_norm_estimate, "hectares\n")
```
...
```{r, message=F, warning=F, error=F, cache=T, fig.show='hold', out.width="50%", echo=F}
...
# Make sure we measure from forest pixels to forest edge
forest_for_distance <- forest_2024 |> raster::raster()
distance_to_edge <- raster::distance(
  forest_for_distance, "./data/BINARY/distance_to_edge.tif",
  overwrite = T) |> terra::rast()
...
# Clip before scaling
slope <- terra::crop(slope, vect(country))
population <- terra::crop(population, vect(country))
distance_to_edge <- terra::crop(distance_to_edge, vect(country))
distance_to_roads <- terra::crop(distance_to_roads, vect(country))
distance_to_places <- terra::crop(distance_to_places, vect(country))
distance_to_water <- terra::crop(distance_to_water, vect(country))
distance_to_urban <- terra::crop(distance_to_urban, vect(country))
...
distance_to_edge    = normalize_function(distance_to_edge)
distance_to_roads     = normalize_function(distance_to_roads)
distance_to_places    = normalize_function(distance_to_places)
distance_to_water     = normalize_function(distance_to_water)
distance_to_urban     = normalize_function(distance_to_urban)


# Invert covariates for increasing risk with increasing proximity
distance_to_edge_inv <- 1 - distance_to_edge
...
distance_to_water_inv <- 1 -  distance_to_water
writeRaster(distance_to_water, "./data/BINARY/distance_to_water_inverted.tif", overwrite=T)
...
```{r, echo=F, cache=T, fig.show='hold', out.width="25%"}
deforestation_yr_zone <- terra::rast("./data/LULC/annual_loss_2014_2024_zonal.tif")
...
terra::plot(deforestation_yr_zone)
...
writeRaster(risk_index_norm, "./data/BINARY/deforestation_risk.tif",overwrite=T)
